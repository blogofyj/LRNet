...
from attack_CR import ContrastLoss
...
def My_loss_backward(x, y, model, label):
    ...
    c_loss = contrast_loss(x_samples, x)

    l_back_rec = Reconstruction_back(x, x_samples)
    l_back_SSIM = Rec_back_SSIM(x, x_samples).mean()
    # l_grad_back_rec = 0.1 * Rec_back_grad(x, x_samples_image)

    logit, _ = model(x_samples)
    logit = torch.sigmoid(logit)
    cla_err = crossLoss(logit, label)

    # return l_back_rec + l_grad_back_rec + l_back_SSIM
    # total_loss = l_back_rec + l_grad_back_rec + l_back_SSIM - cla_err
    # total_loss = l_back_rec + l_back_SSIM + l_grad_back_rec - cla_err
    total_loss = 4*l_back_rec - cla_err - c_loss + l_back_SSIM
    return total_loss, l_back_rec, cla_err
...
def train(epochs, dataloader, loader_test):
    ...
    for epoch in range(1, epochs + 1):
        for data, label in dataloader:
            ...
            # LR, _ = pinglvyu(data)
            # LR = netG.low_conv(LR)
            _, object_semantic = classifier(data)
            object_semantic = object_semantic.view(object_semantic.size(0), -1, 16, 16)

            l_forw_fit = Reconstruction_back(output[:, :2, :, :], object_semantic)

            y_ = torch.cat((output[:, 2:, :, :], gaussian_batch(output[:, :2, :, :].shape)), dim=1)

            # x_samples = netG(x=y_, rev=True)



            # total_loss, l_back_rec, l_grad_back_rec, l_back_SSIM, pertual_loss = My_loss_backward(data, y_, classifier, label)
            total_loss, l_back_rec, cla_err = My_loss_backward(data, y_, classifier, label)
...
if __name__ == '__main__':
    ...
    model_dir = './attack_semantic_sep2'
