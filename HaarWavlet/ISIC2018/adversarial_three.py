...
def add_adv(model, image, label, adv, i=0, default=False):
    ...
    if adv == 'fgsm':
        if default:
            ...
            fgsm_attack = GradientSignAttack(model, eps=0.03)
        ...
        bim_attack = torchattacks.BIM(model, eps=6/255)
    ...
    # projected gradient sign method
        ...
        # print("hi")
        pgd = LinfPGDAttack(model, eps=0.03)
        adv_image = pgd(image)
    ...
    # Carlini-Wagner attack
        ...
        cw_attack = CarliniWagnerL2Attack(model, 7, confidence=1, max_iterations=200, initial_const=1 )
        ...
        # aan_attack = torchattacks.AutoAttack(model, eps=0.03, version='rand')
        aan_attack = torchattacks.AutoAttack(model, eps=6/255, version='standard')
