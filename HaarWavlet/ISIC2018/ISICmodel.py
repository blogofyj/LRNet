...
def show_images(x, x1,x2,x3,x4):
    ...
    for i in range(5):
        ...
        axes[0, 0].imshow(x[i].cpu().numpy().transpose((1,2,0)))
        axes[0, 0].set_title("Clean")
        axes[0, 1].imshow(x[i].cpu().numpy().transpose((1, 2, 0)))
        axes[0, 1].set_title("Clean")
        axes[0, 2].imshow(x[i].cpu().numpy().transpose((1, 2, 0)))
