...
from HaarWavlet.MNIST.Vgg_loss import VGGLoss
...
class Gradient_Loss(nn.Module):
    def __init__(self, losstype='l2'):
        ...
        conv1 = nn.Conv2d(1, 1, kernel_size=3, stride=1, padding=1, bias=False, groups=1)
        ...
        conv2 = nn.Conv2d(1, 1, kernel_size=3, stride=1, padding=1, bias=False, groups=1)
...
def train(epochs, dataloader, loader_test):
    ...
    for epoch in range(1, epochs + 1):
        for data, label, adv_data in dataloader:
            ...
            # clean = netG(data)
            # clean_features = netG.low_clean(clean)

            logit = netG.low_conv(output[:, :1, :, :])
            cla_loss = cel(logit, label)
            # adv_clean_loss = Reconstruction_back(output[:, :3, :, :], clean_features)
            LR, _ = pinglvyu(data)
            LR = netG.low_fre(LR)
            l_forw_fit = Reconstruction_back(output[:, :1, :, :], LR)

            # print('shape', output[:, 0, :, :].shape, output[:, 1:, :, :].shape)

            y_ = torch.cat((output[:, :1, :, :], gaussian_batch(output[:, 1:, :, :].shape)), dim=1)

            total_loss, l_back_rec, l_grad_back_rec, l_back_SSIM, pertual_loss = My_loss_backward(data, y_)

            #total loss
            loss = total_loss + cla_loss + l_forw_fit
            loss.backward()

            nn.utils.clip_grad_norm_(netG.parameters(), 10)
            optimizer_G.step()

            low_F.append(cla_loss.cpu().item())
...
if __name__ == '__main__':
    ...
    model_dir = './New_haar_cla_noBN_Feature_extractor_1_channel_fc100'
